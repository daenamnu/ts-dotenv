...
describe('load', () => {
    ...
    describe('custom location', () => {
        it('should load .env from a custom location', () => {
            ...
            const env = (0, load_1.load)(schema, '.env.custom');
            expect(env).toEqual({ CUSTOM: 123 });
        });
        it('should load .env from a custom location (via options)', () => {
            const schema = { CUSTOM: Number };
            const env = (0, load_1.load)(schema, { path: '.env.custom' });
            expect(env).toEqual({ CUSTOM: 123 });
        });
        it('should not error even when .env is missing (if schema validates)', () => {
            ...
            const env = (0, load_1.load)(schema, '.env.missing');
    ...
    describe('process.env', () => {
        it('should merge loaded .env with process.env', () => {
            ...
            const env = (0, load_1.load)(schema, '.env.partial');
        ...
        it('should allow .env to override process.env (by option)', () => {
            process.env.A = 'false';
            const schema = { A: Boolean };
            const env = (0, load_1.load)(schema, { path: '.env.partial', overrideProcessEnv: true });
